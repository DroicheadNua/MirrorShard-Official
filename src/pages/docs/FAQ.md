...
### Q. なんでAIの動作が遅いんですか？　他のAIエディタはすぐに文章の続きを考えてくれるのに。  

**A.** 設計思想の違いです。  
商用のAIエディタがリアルタイムで補完候補を出すのは、バックグラウンドで常に執筆内容を監視し、クラウドに送信し続けているからです。  
MirrorShardのAI機能は「AIに生殺与奪の権を握らせるな」という設計思想ですので、送信ボタンを押した時点で初めてAIにリクエストが送られます。そのため返答が返ってくるまでに数秒の待ち時間が発生しますが、それはユーザーのプライバシーとPCのリソースを優先しているからです。MirrorShardにおいては、ユーザーが明示的に呼び出すまでAIには何もさせません。  
...
A. MirrorShardの7％くらいはRustでできているからです。  